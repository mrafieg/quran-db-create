...
func GenerateAyahInfoEn() {
	// create scanner
	infoHtml, err := os.Open("data/ayah-info-en.md")
	if err != nil {
		log.Fatal(err)
	}
	...
	for i, info := range ayahInfos {
		if i > 0 {
			query += strings.TrimSpace(fmt.Sprintf(`UPDATE quran_ayahs SET en_ayah_info = "%s" WHERE id = %v;`, Addslashes(info), i)) + "\n"
		}
	}
	...
	fmt.Println("ayah-info-en.sql written successfully.")
}
