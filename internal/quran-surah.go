...
func GenerateQuranSurah() {
	...
	var resultSurahNameEn map[int]SurahNameEn
	...
	var surahDB []SurahDBItem = make([]SurahDBItem, 1)
	var surahIndex = 0

	for i := 1; i <= len(resultWord); i++ {
		if value := resultWord[i]; value.Surah != surahIndex {
			surahIndex = value.Surah
			surahDB = append(surahDB,
				SurahDBItem{
					SurahId:         value.Surah,
					SurahName:       resultSurat.Data[value.Surah-1].Name, // resultSurat.Data is array so start index 0
					ArabicName:      resultSurah[surahIndex].Name,
					IdnName:         resultSurat.Data[value.Surah-1].NameTranslate, // resultSurat.Data is array so start index 0
					EnName:          resultSurahNameEn[surahIndex].Translation,
					Type:            resultSurah[surahIndex].Type,
					TotalAyah:       resultSurah[surahIndex].TotalAyah,
					WordCount:       0,
					RevelationOrder: resultSurah[surahIndex].RevelationOrder,
					AyahStart:       resultSurah[surahIndex].AyahStart,
					AyahEnd:         resultSurah[surahIndex].AyahEnd,
				})
		}
		surahDB[surahIndex].WordCount += 1
	}
	...
	for i, item := range surahDB {
		if i > 0 {
			newQuery := fmt.Sprintf(`INSERT INTO quran_surahs (id, surah_name, arabic_name, idn_name, en_name, surah_type, total_ayah, word_count, revelation_order, ayah_start, ayah_end) VALUES (%v,"%s","%s","%s","%s","%s",%v,%v,%v,%v,%v);`, item.SurahId, item.SurahName, item.ArabicName, item.IdnName, item.EnName, item.Type, item.TotalAyah, item.WordCount, item.RevelationOrder, item.AyahStart, item.AyahEnd)
			query += newQuery + "\n"
		}
	}
