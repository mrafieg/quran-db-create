...
func GenerateSurahInfoEn() {
	// create scanner
	infoHtml, err := os.Open("data/surah-info-en.md")
	if err != nil {
		log.Fatal(err)
	}
	...
	for scanner.Scan() {
		// transform md line to html tags
	...
	for i, info := range surahInfos {
		if i > 0 {
			query += fmt.Sprintf(`UPDATE quran_surahs SET en_surah_info = "%s" WHERE id = %v;`, Addslashes(info), i) + "\n"
		}
	}

	// write sql file
	err3 := os.WriteFile("sql/5_surah-info-en.sql", []byte(query), 0777)
	if err3 != nil {
		log.Fatal(err3)
	}

	fmt.Println("surah-info-en.sql written successfully.")
}
