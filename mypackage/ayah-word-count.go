	...
	for i, surah := range surahs {
		if i > 0 {
			for _, ayah := range surah.Ayahs {
				query += fmt.Sprintf(`UPDATE quran_ayahs SET word_count = %s WHERE surah_id = %s AND verse_id = %s`, strconv.Itoa(ayah.WordCount), strconv.Itoa(surah.Id), strconv.Itoa(ayah.VerseId)) + ";\n"
			}
		}
	}
