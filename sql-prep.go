...
func main() {
	// read word-data.json
	wordData, err := os.ReadFile("word-data.json")
	if err != nil {
		log.Fatal(err)
	}
	type Word struct {
	...
	surahData, err2 := os.ReadFile("surah-data.json")
	if err2 != nil {
		log.Fatal(err2)
	}
	type Surah struct {
	...
	surahDataIndo, err3 := os.ReadFile("surah-data-indo.json")
	if err3 != nil {
		log.Fatal(err3)
	}
